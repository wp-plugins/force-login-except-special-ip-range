...
add_action ( 'init', 'force_login_except_ip_range' );

function force_login_except_ip_range($content) {
	$allowed_ip_ranges=array();
	require_once("allowed_ip_ranges.php");
	#Of course wp-login.php must not be redirected! 
	if(!(substr(strrev($_SERVER['PHP_SELF']),0,strlen("wp-login.php")) == strrev("wp-login.php"))) {
		# if user ist not logged in, check if is in IP Range
        	if(!is_user_logged_in()) {
			// check the ip ranges
			foreach($allowed_ip_ranges as $range) {
				$ip=split(",", $range);
				//echo "check ".trim($ip[0]).",".trim($ip[1])."<br>";
				if(in_ip_range(trim($ip[0]), trim($ip[1]))) {
...
function redirect_to_login() {
	...
	$url.='wp-login?redirect_to=' . $redirect_to;
	header( 'Location: '.$url );
	exit;
}
